describe("vec4", async function() {
    const {Vector3: vec3, Vector4: vec4} = import('../pkg/gl_matrix_wasm.js');
    const x = import('../pkg/gl_matrix_wasm.js');
    console.log(x, vec3, vec4);
    let out, vecA, vecB, result;

    beforeEach(function() {
        vecA = vec3.fromValues(1, 2, 3, 4);
        vecB = vec3.fromValues(5, 6, 7, 8);
        out = vec3.fromValues(0, 0, 0, 0);
    });
